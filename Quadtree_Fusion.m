function [fimg, Fusion_decision_map] = Quadtree_Fusion(name, type, num, level)
%--------------------------------------------------------------------------
% Quadtree based image fusion with the weighted focus-measure
%--------------------------------------------------------------------------

% ----------------- Get dimensions of multi-focus images ------------------
img = double(imread (strcat('example images\', name, num2str(1), type)));
[p1,p2,p3] = size(img);
...
for q=1:num
    img = double(imread (strcat('example images\', name, num2str(q), type)));
    mImg(:,:,q)=img;
end
